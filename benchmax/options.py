...
def parse_memout(mem_str: str) -> int:
    ...
    match parts.group(2):
        case "K": return int(parts.group(1))
        case "M": return int(parts.group(1)) * 1000
        case "G": return int(parts.group(1)) * 1000_000
...
def parse_options(cmdlineoptions=None):
    ...
    output_group.add_argument(
        "-s", "--statistics",
        help="collect statistics if possible",
        action="store_true"
    )
